...
fn preprocess_extension_flags(raw_args: &[String]) -> (Vec<String>, Vec<ExtensionCliFlag>) {
    if raw_args.is_empty() {
        return (vec!["skaffen".to_string()], Vec::new());
    }
...
pub fn parse_with_extension_flags(raw_args: Vec<String>) -> Result<ParsedCli, clap::Error> {
    if raw_args.is_empty() {
        let cli = Cli::try_parse_from(["skaffen"])?;
        return Ok(ParsedCli {
            cli,
...
mod tests {
    ...
    fn parse_continue_short_flag() {
        let cli = Cli::parse_from(["skaffen", "-c"]);
    ...
    fn parse_continue_long_flag() {
        let cli = Cli::parse_from(["skaffen", "--continue"]);
        assert!(cli.r#continue);
    }

    #[test]
    fn parse_resume_short_flag() {
        let cli = Cli::parse_from(["skaffen", "-r"]);
        assert!(cli.resume);
        assert!(!cli.r#continue);
    }
    ...
    fn parse_print_long_flag() {
        let cli = Cli::parse_from(["skaffen", "--print", "question"]);
        assert!(cli.print);
    }

    #[test]
    fn parse_model_flag() {
        let cli = Cli::parse_from(["skaffen", "--model", "claude-opus-4"]);
        assert_eq!(cli.model.as_deref(), Some("claude-opus-4"));
    }

    #[test]
    fn parse_provider_flag() {
        let cli = Cli::parse_from(["skaffen", "--provider", "openai"]);
        assert_eq!(cli.provider.as_deref(), Some("openai"));
    }

    #[test]
    fn parse_api_key_flag() {
        let cli = Cli::parse_from(["skaffen", "--api-key", "sk-ant-test123"]);
        assert_eq!(cli.api_key.as_deref(), Some("sk-ant-test123"));
    }

    #[test]
    fn parse_version_short_flag() {
        let cli = Cli::parse_from(["skaffen", "-v"]);
        assert!(cli.version);
    }

    #[test]
    fn parse_version_long_flag() {
        let cli = Cli::parse_from(["skaffen", "--version"]);
        assert!(cli.version);
    }

    #[test]
    fn parse_with_extension_flags_preserves_help_error() {
        let err = parse_with_extension_flags(vec!["skaffen".into(), "--help".into()])
    ...
    fn parse_verbose_flag() {
        let cli = Cli::parse_from(["skaffen", "--verbose"]);
        assert!(cli.verbose);
    }
    ...
    fn parse_export_flag() {
        let cli = Cli::parse_from(["skaffen", "--export", "output.html"]);
    ...
    fn parse_all_thinking_levels() {
        for level in &["off", "minimal", "low", "medium", "high", "xhigh"] {
            let cli = Cli::parse_from(["skaffen", "--thinking", level]);
    ...
    fn invalid_thinking_level_rejected() {
        let result = Cli::try_parse_from(["skaffen", "--thinking", "ultra"]);
    ...
    fn file_and_message_args_split() {
        let cli = Cli::parse_from(["skaffen", "@a.txt", "hello", "@b.md", "world"]);
    ...
    fn file_args_empty_when_none() {
        let cli = Cli::parse_from(["skaffen", "hello", "world"]);
    ...
    fn message_args_empty_when_only_files() {
        let cli = Cli::parse_from(["skaffen", "@src/main.rs", "@Cargo.toml"]);
    ...
    fn no_positional_args_yields_empty() {
        let cli = Cli::parse_from(["skaffen"]);
        assert!(cli.file_args().is_empty());
        assert!(cli.message_args().is_empty());
    }
    ...
    fn at_prefix_stripped_from_file_paths() {
        let cli = Cli::parse_from(["skaffen", "@/absolute/path.rs"]);
    ...
    fn parse_install_subcommand() {
        let cli = Cli::parse_from(["skaffen", "install", "npm:@org/pkg"]);
        match cli.command {
    ...
    fn parse_install_local_flag() {
        let cli = Cli::parse_from(["skaffen", "install", "--local", "git:https://example.com"]);
        match cli.command {
            Some(Commands::Install { source, local }) => {
    ...
    fn parse_install_local_short_flag() {
        let cli = Cli::parse_from(["skaffen", "install", "-l", "./local-ext"]);
        match cli.command {
    ...
    fn parse_remove_subcommand() {
        let cli = Cli::parse_from(["skaffen", "remove", "npm:pkg"]);
        match cli.command {
    ...
    fn parse_remove_local_flag() {
        let cli = Cli::parse_from(["skaffen", "remove", "--local", "npm:pkg"]);
        match cli.command {
    ...
    fn parse_update_with_source() {
        let cli = Cli::parse_from(["skaffen", "update", "npm:pkg"]);
        match cli.command {
    ...
    #[test]
    fn parse_update_all() {
        let cli = Cli::parse_from(["skaffen", "update"]);
        match cli.command {
    ...
    fn parse_list_subcommand() {
        let cli = Cli::parse_from(["skaffen", "list"]);
        assert!(matches!(cli.command, Some(Commands::List)));
    }

    #[test]
    fn parse_config_subcommand() {
        let cli = Cli::parse_from(["skaffen", "config"]);
        match cli.command {
    ...
    fn parse_config_show_flag() {
        let cli = Cli::parse_from(["skaffen", "config", "--show"]);
        match cli.command {
            Some(Commands::Config { show, paths, json }) => {
                assert!(show);
                assert!(!paths);
    ...
    fn parse_config_paths_flag() {
        let cli = Cli::parse_from(["skaffen", "config", "--paths"]);
        match cli.command {
            Some(Commands::Config { show, paths, json }) => {
                assert!(!show);
                assert!(paths);
    ...
    fn parse_config_json_flag() {
        let cli = Cli::parse_from(["skaffen", "config", "--json"]);
        match cli.command {
            Some(Commands::Config { show, paths, json }) => {
                assert!(!show);
                assert!(!paths);
    ...
    fn parse_update_index_subcommand() {
        let cli = Cli::parse_from(["skaffen", "update-index"]);
        assert!(matches!(cli.command, Some(Commands::UpdateIndex)));
    }

    #[test]
    fn parse_info_subcommand() {
        let cli = Cli::parse_from(["skaffen", "info", "auto-commit-on-exit"]);
        match cli.command {
    ...
    fn no_subcommand_when_only_message() {
        let cli = Cli::parse_from(["skaffen", "hello"]);
    ...
    fn list_models_without_pattern() {
        let cli = Cli::parse_from(["skaffen", "--list-models"]);
        assert!(matches!(cli.list_models, Some(None)));
    }

    #[test]
    fn list_models_with_pattern() {
        let cli = Cli::parse_from(["skaffen", "--list-models", "claude*"]);
    ...
    fn list_providers_set() {
        let cli = Cli::parse_from(["skaffen", "--list-providers"]);
        assert!(cli.list_providers);
    }
    ...
    fn unknown_flag_rejected() {
        let result = Cli::try_parse_from(["skaffen", "--nonexistent"]);
        assert!(result.is_err());
    }

    #[test]
    fn invalid_mode_rejected() {
        let result = Cli::try_parse_from(["skaffen", "--mode", "xml"]);
        assert!(result.is_err());
    }

    #[test]
    fn install_without_source_rejected() {
        let result = Cli::try_parse_from(["skaffen", "install"]);
        assert!(result.is_err());
    }

    #[test]
    fn remove_without_source_rejected() {
        let result = Cli::try_parse_from(["skaffen", "remove"]);
        assert!(result.is_err());
    }

    #[test]
    fn invalid_subcommand_option_rejected() {
        let result = Cli::try_parse_from(["skaffen", "install", "--bogus", "npm:pkg"]);
        assert!(result.is_err());
    }
    ...
    #[test]
    fn multiple_skills() {
        let cli = Cli::parse_from(["skaffen", "--skill", "a.md", "--skill", "b.md"]);
        assert_eq!(
            cli.skill,
            vec!["a.md", "b.md"]
                .into_iter()
    ...
    fn multiple_theme_paths() {
        let cli = Cli::parse_from(["skaffen", "--theme-path", "a/", "--theme-path", "b/"]);
        assert_eq!(
            cli.theme_path,
            vec!["a/", "b/"]
                .into_iter()
    ...
    fn no_extensions_flag() {
        let cli = Cli::parse_from(["skaffen", "--no-extensions"]);
        assert!(cli.no_extensions);
    }

    #[test]
    fn no_skills_flag() {
        let cli = Cli::parse_from(["skaffen", "--no-skills"]);
        assert!(cli.no_skills);
    }

    #[test]
    fn no_prompt_templates_flag() {
        let cli = Cli::parse_from(["skaffen", "--no-prompt-templates"]);
    ...
    fn explain_extension_policy_flag_parses() {
        let cli = Cli::parse_from(["skaffen", "--explain-extension-policy"]);
    ...
    fn repair_policy_flag_parses() {
        let cli = Cli::parse_from(["skaffen", "--repair-policy", "auto-safe"]);
        assert_eq!(cli.repair_policy.as_deref(), Some("auto-safe"));
    }

    #[test]
    fn repair_policy_flag_off() {
        let cli = Cli::parse_from(["skaffen", "--repair-policy", "off"]);
    ...
    fn explain_repair_policy_flag_parses() {
        let cli = Cli::parse_from(["skaffen", "--explain-repair-policy"]);
    ...
    fn ts_parity_short_flags_match() {
        ...
        let cli = Cli::parse_from(["skaffen", "-c", "-p", "-e", "ext.js"]);
        assert!(cli.r#continue);
        assert!(cli.print);
        assert_eq!(cli.extension, vec!["ext.js"]);

        let cli2 = Cli::parse_from(["skaffen", "-r"]);
        assert!(cli2.resume);
    }
    ...
    fn ts_parity_subcommands() {
        ...
        let cli = Cli::parse_from(["skaffen", "install", "npm:my-ext"]);
        assert!(matches!(cli.command, Some(Commands::Install { .. })));

        let cli = Cli::parse_from(["skaffen", "remove", "npm:my-ext"]);
    ...
    fn ts_parity_at_file_expansion() {
        let cli = Cli::parse_from(["skaffen", "-p", "@readme.md", "summarize this"]);
    ...
    fn ts_parity_list_models_optional_search() {
        ...
        let cli = Cli::parse_from(["skaffen", "--list-models", "sonnet"]);
    ...
    mod proptest_cli {
        ...
        proptest! {
            ...
            fn preprocess_known_flags_never_extracted(
                ...
                let args: Vec<String> = vec!["skaffen".to_string(), flag.to_string()];
            ...
            fn preprocess_unknown_flags_are_extracted(
                ...
                let args: Vec<String> = vec!["skaffen".to_string(), flag.clone()];
                let result = preprocess_extension_flags(&args);
                assert!(
            ...
            fn preprocess_double_dash_terminates(
                ...
                let mut args = vec!["skaffen".to_string(), "--".to_string()];
