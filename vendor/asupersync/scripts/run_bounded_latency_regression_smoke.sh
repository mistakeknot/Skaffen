...
# - bounded-latency-regression-smoke-bundle-v1
# - bounded-latency-regression-smoke-run-report-v1

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
CONTRACT_ARTIFACT="${PROJECT_ROOT}/artifacts/bounded_latency_regression_v1.json"
OUTPUT_ROOT="${BLR_SMOKE_OUTPUT_DIR:-${PROJECT_ROOT}/target/bounded-latency-regression-smoke}"
...
usage() {
    cat <<'USAGE'
Usage: ./scripts/run_bounded_latency_regression_smoke.sh [options]
...
echo ""
echo "==================================================================="
echo "     BOUNDED LATENCY REGRESSION SMOKE SUMMARY                      "
echo "==================================================================="
