...
TEST_FILTER="${1:-}"
RCH_BIN="${RCH_BIN:-rch}"
WORKLOAD_ID="${WORKLOAD_ID:-AA01-WL-IO-001}"
RUNTIME_PROFILE="${RUNTIME_PROFILE:-native-e2e}"
WORKLOAD_CONFIG_REF="${WORKLOAD_CONFIG_REF:-scripts/test_transport_e2e.sh::e2e_transport/all_features}"
...
if ! run_cargo check --test e2e_transport --all-features 2>"${ARTIFACT_DIR}/compile_errors.log"; then
