...
# bead: asupersync-3qv04.6.1

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd -- "${SCRIPT_DIR}/.." && pwd)"
FIXTURE_DIR="${REPO_ROOT}/tests/fixtures/vite-vanilla-consumer"
RESULT_ROOT="${REPO_ROOT}/target/e2e-results/vite_vanilla_consumer"
...
summary = {
    ...
    "timestamp": timestamp,
    "fixture": "tests/fixtures/vite-vanilla-consumer",
    "status": "pass",
    "checks": {
...
cat <<EOF
Vite vanilla consumer validation passed.
Artifacts:
  log: ${LOG_FILE}
