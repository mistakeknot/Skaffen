...
use skaffen::error_hints::{format_error_with_hints, hints_for_error};
use skaffen::model::{ContentBlock, ImageContent};
use skaffen::tools::{TruncatedBy, truncate_head, truncate_tail};
use skaffen::vcr::{
...
fn apply_piped_stdin_none_does_nothing() {
    let mut cli = Cli::parse_from(["skaffen", "hello"]);
...
fn apply_piped_stdin_empty_string_does_nothing() {
    let mut cli = Cli::parse_from(["skaffen"]);
...
fn apply_piped_stdin_prepends_to_existing_args() {
    let mut cli = Cli::parse_from(["skaffen", "existing_arg"]);
...
fn normalize_cli_print_sets_no_session() {
    let mut cli = Cli::parse_from(["skaffen", "-p", "hello"]);
    assert!(cli.print);
...
fn normalize_cli_lowercases_provider() {
    let mut cli = Cli::parse_from(["skaffen", "--provider", "OpenAI"]);
    app::normalize_cli(&mut cli);
...
fn validate_rpc_args_no_mode_is_ok() {
    let cli = Cli::parse_from(["skaffen", "@file.txt"]);
    assert!(app::validate_rpc_args(&cli).is_ok());
}

#[test]
fn validate_rpc_args_rpc_mode_no_files_is_ok() {
    let cli = Cli::parse_from(["skaffen", "--mode", "rpc", "hello"]);
    assert!(app::validate_rpc_args(&cli).is_ok());
}

#[test]
fn validate_rpc_args_rpc_mode_with_files_is_error() {
    let cli = Cli::parse_from(["skaffen", "--mode", "rpc", "@file.txt"]);
...
fn validate_rpc_args_text_mode_with_files_is_ok() {
    let cli = Cli::parse_from(["skaffen", "--mode", "text", "@file.txt"]);
    assert!(app::validate_rpc_args(&cli).is_ok());
}
