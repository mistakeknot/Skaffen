...
mod cli_combinations {
    ...
    fn print_mode_with_json_output() {
        let cli = Cli::parse_from(["skaffen", "-p", "--mode", "json"]);
        assert!(cli.print);
    ...
    fn tools_subset_with_provider() {
        let cli = Cli::parse_from(["skaffen", "--tools", "read,bash", "--provider", "anthropic"]);
    ...
    fn trailing_message_with_at_file_ref() {
        let cli = Cli::parse_from(["skaffen", "-p", "hello", "@file.txt", "world"]);
        assert!(cli.print);
    ...
    fn extension_policy_profiles() {
        for profile in &["safe", "balanced", "permissive"] {
            let cli = Cli::parse_from(["skaffen", "--extension-policy", profile]);
    ...
    fn all_short_flags_together() {
        ...
        let cli = Cli::parse_from(["skaffen", "-p", "-c"]);
        assert!(cli.print);
        assert!(cli.r#continue);
    }
    ...
    fn repair_policy_modes() {
        for mode in &["off", "suggest", "auto-safe", "auto-strict"] {
            let cli = Cli::parse_from(["skaffen", "--repair-policy", mode]);
